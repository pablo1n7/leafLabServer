...
_bsn.AutoSuggest = function (fld, param)
{
	// no DOM - give up!
	//
	...
	if (!this.oP.maxentries)								this.oP.maxentries = 3;
...
_bsn.AutoSuggest.prototype.getSuggestions = function (val)
{
	...
	if (this.aSuggestions.length && this.oP.cache)
	{
		...
		for (var i=0;i<this.aSuggestions.length;i++)
		{
            ...
            if (this.aSuggestions[i][2].match(re) != null && (arr.length<this.oP.maxentries))
				arr.push( this.aSuggestions[i] );
		}
...
_bsn.AutoSuggest.prototype.setSuggestions = function (req)
{
	...
	if (this.fld.id.length == 0) {
		this.idAs = "as_"+this.fld.name;
		return;
	};
...
document.getElementsByTagName("body")[0].appendChild(div);
	$(div).css({"top":$(this.fld).offset().top+$(this.fld).height()+10+"px"});
